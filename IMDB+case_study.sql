...
SELECT
	COUNT(title) AS produced_movies_count
FROM
	movie
...
select 
  count(genre_count) as single_movie_count 
from 
  single_genre_count;
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
WITH company_rank_on_avg_rating AS
(
SELECT 
...
SELECT *
FROM company_rank_on_avg_rating
...
SELECT
	m.country,
	SUM(r.total_votes) AS total_votes_count
FROM
	ratings AS r
	INNER JOIN movie AS m
    ON r.movie_id = m.id
...
SELECT 
    ...
    RANK() OVER(ORDER BY COUNT(n.id) DESC) AS DIR_RANK
FROM 
	names as n
...
SELECT
    ...
    AVG(AVG(duration)) OVER(ORDER BY genre ROWS 1 PRECEDING) AS moving_avg_duration
FROM genre AS g 
	INNER JOIN movie AS m 
    ON m.id = g.movie_id
GROUP BY genre;
...
(
	...
	FROM 
		...
		INNER JOIN genre AS g 
        ON m.id= g.movie_id
	WHERE genre IN (SELECT genre FROM selected_genre)
)
SELECT *
FROM top_five_movie
WHERE movie_rank<=5;
